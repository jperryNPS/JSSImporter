...
## added for jamf patch integration - start
import base64
import json
import ssl
import urllib
import urllib2
## added for jamf patch integration - end
...
#Define patch server and if needed location
#Set PATCH_SERVER_ENABLED to 'Local' if running autopkg on the same system as your json file or 'SSH' if running on a seperate system

"PATCH_SERVER_ENABLED": {
    "required": False,
    "Description":
    "Enable in order to use the Patch Server option to update json on a local or remote server"
    "Set to Disable if not using"
    "'False'. Defaults to 'Disable'.",
    "default": Disable,
}
"PATCH_SERVER": {
    "required": False,
    "description":
        "The server that hosts your third party patch metadata",
}
"PATCH_SERVER_USERNAME": {
    "required": False,
    "description":
    "The username to SSH into the server that hosts your third party patch json file",
}
"PATCH_SERVER_PASSWORD": {
    "required": False,
    "Description":
    "The password to SSH into the server that hosts your third party patch json file",
}
"PATCH_SERVER_PATH": {
    "required": False,
    "Description":
    "The relative path to the patch server.
    "If using SSH would be relative to the SSH home directory"
    "Use the full URI if local",
    "default": /,
}
    output_variables = {
        "jss_changed_objects": {
...
################################################################################
# added to output package info for jamf patch - start
baseUrl = $PATCH_SERVER
    PlistBuddyBin = '/usr/libexec/PlistBuddy'
        attribute = []
        jp_json = {"currentVersion":"","bundleId":"","appName":"", "lastModified":"", "minimum_os_version":""}
            
            p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            attribute = p1.communicate()[0]
            sections = attribute.count('Output') - 1
            for i in range(1, sections):
                p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Processor', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                attribute = p1.communicate()[0].replace('\n','')
                if attribute == 'AppDmgVersioner':
                    p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:version', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    attribute = p1.communicate()[0]
                    jp_json['currentVersion'] = attribute.replace('\n','')
                    #jp_version = attribute[0].replace('\n','')
                    
                    p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:bundleId', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        attribute = p1.communicate()[0]
                        jp_json['bundleId'] = attribute.replace('\n','')
                        
                        p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:app_name', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        attribute = p1.communicate()[0]
                        jp_json['appName'] = attribute.replace('\n','')
                    elif attribute == 'URLDownloader':
                        p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:last_modified', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        attribute = p1.communicate()[0]
                        jp_json['lastModified'] = attribute.replace('\n','')
                    else:
                        p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:version', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        attribute = p1.communicate()[0]
                        if jp_json['currentVersion'] == '':
                            jp_json['currentVersion'] = attribute.replace('\n','')
                        
                        
                        p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:minimum_os_version', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        attribute = p1.communicate()[0]
                        if jp_json['minimum_os_version'] == '':
                            jp_json['minimum_os_version'] = attribute.replace('\n','')
        
            # p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print '+str(i)+':Output:last_modified', receipt_path], stdout=subprocess.PIPE)
            # attribute = p1.communicate()[0]
            # if jp_json['lastModified'] == '':
            #       jp_json['lastModified'] = attribute.replace('\n','')
            
            p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print 0:Recipe\ input:NAME', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                attribute = p1.communicate()[0]
                jp_json['name'] = attribute.replace('\n','')
                jp_json['id'] = "".join(jp_json['name'].split())
                
                p1 = subprocess.Popen([PlistBuddyBin,'-c', 'Print 0:Recipe\ input:RECIPE_DIR', receipt_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                attribute = p1.communicate()[0].split('/')
                jp_json['publisher'] = attribute[-1].replace('\n','')
                app_json = str(jp_json).replace('\'','"')
                print('\n'+app_json+'\n')
                
                ## post results to server
                softwareTitle = jp_json['name']
                print('softwareTitle: ' + softwareTitle)
                # theTitle = str(urllib.urlencode(softwareTitle))
                # print('theTitle: ' + theTitle)
                newApp = urllib2.Request(baseUrl)
                newApp.add_header("Accept", "application/json")
                newApp.add_header("Content-Type", "application/json")
                result = urllib2.urlopen(newApp, app_json)
                html = result.read()
                print html

# added to output package info for jamf patch - end
################################################################################
